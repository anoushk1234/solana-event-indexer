...
import { DataEmitter } from "../target/types/data_emitter";

describe("data-emitter", () => {
  // Configure the client to use the local cluster.
  anchor.setProvider(anchor.AnchorProvider.env());

  const program = anchor.workspace.DataEmitter as Program<DataEmitter>;
  ...
  it.skip("Multiple events", async () => {
    ...
    console.log(slotOne, eventOne);
    console.log(slotTwo, eventTwo);
  ...
  it.skip("Multiple events2", async () => {
    // Sleep so we don't get this transaction has already been processed.
    await sleep(2000);
